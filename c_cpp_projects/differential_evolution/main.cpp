...
std::vector<double> selection(std::vector<double> &X, std::vector<double> &Xnew, int D, int i, std::vector<double> &values, double &bestX, void (*f)(double*, double*, int, int, int)) {
    double fOdX;
    double fOdX1;
    
    f(&Xnew[0], &fOdX, (int)D, 1, 1);
    f(&X[0], &fOdX1, (int)D, 1, 1);
...
int main(int argc, const char * argv[]) {
    if(argc > 3) { std::cout << "Too many arguments" << std::endl; return 0; }
...
//        start measuring time
        ...
        diff_evo(values, 9, MAX_NFES);
//        end measuring time
        end = std::chrono::system_clock::now();
        auto finalTime = std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
