...
void verboseData(int iteration, int threadNum, std::chrono::system_clock::time_point start, int NFES, double bestX, double bestGlobalX, int id) {
    auto end = std::chrono::system_clock::now();
    auto timeToSend = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    if(id == 0) {
        ...
        MPI_Send(&threadNum, 1, MPI_INT, 0, iteration, MPI_COMM_WORLD);
        MPI_Send(&iteration, 1, MPI_INT, 0, iteration, MPI_COMM_WORLD);
        MPI_Send(&timeToSend, 1, MPI_INT, 0, iteration, MPI_COMM_WORLD);
        MPI_Send(&NFES, 1, MPI_INT, 0, iteration, MPI_COMM_WORLD);
        MPI_Send(&bestX, 1, MPI_DOUBLE, 0, iteration, MPI_COMM_WORLD);
        MPI_Send(&bestGlobalX, 1, MPI_DOUBLE, 0, iteration, MPI_COMM_WORLD);
        MPI_Send(&id, 1, MPI_INT, 0, iteration, MPI_COMM_WORLD);
    }
}
...
void selection(bool verbose,
    ...
    if (fOdX < p_value) {
        ...
        if(fOdX < bestX) {
            critical.lock();
            bestX = fOdX;
            if(verbose) verboseData(iteration, threadNum, start, NFES, bestX, bestGlobalX, id);
            critical.unlock();
        }
    } //else {
...
//    std::mutex crit;
    ...
    while (NFES < MAX_NFES) {
        for (int i = 0; i < NpSize; i++) {
            ...
            fixing(K, Xnew, D);
            selection(verbose, P[i], Xnew, p_values[i], values, bestX, cec19_test_func, iteration, threadNum, id, start, NFES, bestGlobalX, bestGlobal);
...
void entryPoint(int runs, int id, int threadsLmt, int seed, bool verbose) {
    ...
    double bestGlobalX = INT_MAX; //best f(x) value
    {
        ...
        for(int t = 0; t < threadsLmt; t++) {
            threads[t] = std::thread(std::bind([&](const int bi, const int ei, const int t) {
                ...
                for(int i = bi; i < ei; i++) {
                    ...
                    diff_evo(verbose, threadValues, DIMENSIONS, MAX_NFES, bestGlobalX, bestGlobal, t, i, start, seed, id);
                }
            },t * runs / threadsLmt, (t+1) == threadsLmt ? runs : (t+1) * runs / threadsLmt, t));
        }
        ...
        if(id == 0) {
            dataPrint(threadsLmt, runs, MAX_NFES, finalGeneralTime, bestGlobalX, bestGlobal);
            std::cout << std::endl;
        }
    }
}
...
int main(int argc, const char *argv[]) {
    //parameters parsing
    int proc, id;
    MPI_Init(NULL, NULL);//, MPI_THREAD_MULTIPLE, &prov);
//    if(prov == MPI_THREAD_MULTIPLE) {
//        std::cout << "Success." << std::endl;
//    } else {
//        std::cout << "Unhandled request." << std::endl;
//    }
    ...
    if(id == 0) {
        int num = 0;
        int runs = runsLmt/proc + runsLmt % proc;
        ...
        entryPoint(runs, 0, threadsLmt, seed, verbose);
        ...
        for(int i = 1; i < proc; i++) {
            for(int j = 0; j < num; j++) {
                ...
                double bestX;
                double bestGlobalX;
                MPI_Recv(&threadNum, 1, MPI_INT, i, j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(&iteration, 1, MPI_INT, i, j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(&timeToReceive, 1, MPI_INT, i, j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(&NFES, 1, MPI_INT, i, j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(&bestX, 1, MPI_DOUBLE, i, j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(&bestGlobalX, 1, MPI_DOUBLE, i, j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(&recId, 1, MPI_INT, i, j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                if(verbose) {
                    std::cout << "Current thread: " << threadNum << std::endl;
                    std::cout << "Process ID: " << recId << std::endl;
                    std::cout << "Thread ID: " << std::this_thread::get_id() << std::endl;
                    std::cout << "Run: " << iteration << std::endl;
                    std::cout << "Runtime: " << timeToReceive << "ms" << std::endl;
                    std::cout << "NFES: " << NFES << std::endl;
                    std::cout << "Current best: " << bestX << std::endl;
                    std::cout << "Current program-wide best: " << bestGlobalX << std::endl;
                    std::cout << std::endl;
    ...
    if(id > 0) {
        ...
        entryPoint(num, id, threadsLmt, seed, verbose);
    }
